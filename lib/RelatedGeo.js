!function() {
    ...
    module.exports = new Class({
        ...
        , moveQuery: function(resource, query) {
            ...
            // select all fields of the original query
            query.select.forEach((selection) => {
                if (!(type.function(selection.isRelatedSelector) && selection.isRelatedSelector() && selection.name === 'distanceFrom')) {
                    if (selection.alias) {
                        if (!map[selection.alias]) subQuery.select.push(resource.getORM().sum(selection.alias, selection.alias));
                        map[selection.alias] = true;
                    }
                    else if (selection.column) {
                        if (!map[selection.column]) subQuery.select.push(selection.column);
                        map[selection.column] = true;
                    }
                    else {
                        if (!map[selection]) subQuery.select.push(selection);
                        else query.select.splice(query.select.indexOf(selection), 1);
                        map[selection] = true;
                    }
                    
                    // when a selection has an alias we're not grouping by it, 
                    // we're returning the sum for it.
                    if (!selection.alias) groupBy.push(selection);
                }
                else distanceSelection = selection;
            });


            if (distanceSelection) {
                subQuery.select.push(resource.getORM().min(distanceSelection.alias, distanceSelection.alias));
            }


            // group
            subQuery.group = subQuery.group.concat(groupBy);
